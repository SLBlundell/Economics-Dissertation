...
class DataProcessor:
    ...
    def __init__(self, returns_file, stringency_file):
        ...
        self.df_spec = pd.DataFrame(columns=['date', 'R_m', 'CSAD', 'CSSD', 'stringency', 'lockdown', 'vaccine'])
        """
        Parameters
        ----------
    ...
    def process_data(self):
        ...
        for date in self.df.trade_date.unique().tolist():
            ...
            lockdown = self.df_stringency.loc[self.df_stringency['Date'] == int(date), 'C6E_Stay at home requirements'].item()
            vaccine = self.df_stringency.loc[self.df_stringency['Date'] == int(date), 'PopulationVaccinated'].item()
...
class GetStringencyData:
    ...
    def filter_data(self, df):
        ...
        df = df[['Date', 'StringencyIndex_WeightedAverage', 'C6E_Stay at home requirements', 'PopulationVaccinated']].replace(np.nan, 0)
        return df
